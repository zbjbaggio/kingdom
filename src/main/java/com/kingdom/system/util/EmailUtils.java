...
public class EmailUtils {
    ...
    public static void sendEmail(EmailDTO emailDTO) throws Exception {
        ...
        */
        ...
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, emailDTO);
     ...
     * @throws Exception
     */
    private static MimeMessage createMimeMessage(Session session, EmailDTO emailDTO) throws Exception {
        // 1. 创建一封邮件
        MimeMessage message = new MimeMessage(session);
        // 2. From: 发件人（昵称有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改昵称）
        message.setFrom(new InternetAddress(myEmailAccount, emailDTO.getSendMailPersonal(), CHARSET));
        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(emailDTO.getReceiveMailAccount(), emailDTO.getReceiveMailPersonal(), CHARSET));
        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject(emailDTO.getSubject(), CHARSET);
        // 5. Content: 邮件正文（可以使用html标签）（内容有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改发送内容）
        message.setContent(emailDTO.getContent(), TYPE);
        // 6. 设置发件时间
    ...
    public void setMyEmailSMTPPort(String myEmailSMTPPort) {
        EmailUtils.myEmailSMTPPort = myEmailSMTPPort;
    }
